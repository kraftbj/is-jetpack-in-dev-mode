...
function bk_jp_is_development_mode() {
	...
	$development_mode_status = 'JETPACK: Not in Development Mode';
	$notice_status = "updated";

	if ( defined( 'JETPACK_DEV_DEBUG' ) ) {
		if ( JETPACK_DEV_DEBUG == true) {
			$development_mode_status = "JETPACK: In Dev Mode via wp-config.php or constant being defined elsewhere.";
			$notice_status = "error";
		}
	}
	...
	if ( apply_filters( 'jetpack_development_mode', $development_mode ) == true ) {
		$development_mode_status = "JETPACK: In Dev Mode via an add_filter call in a plugin.";
		$notice_status = "error";
	}
...
add_action( 'admin_notices', 'bk_jp_is_development_mode' );